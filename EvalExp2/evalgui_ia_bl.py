...
class EvalApp(object):
    ...
    def __init__(self, parent,tnumber=16):
        ...
        self.logname = 'final_logs/log_ia_bl_'+str(tnumber)+'_'+str(time.time())+'.txt'
        self.logfile = open(self.logname,'w')
        print 'final_logs/log_ia_bl'+str(tnumber)+'_'+str(time.time())+'.txt'
        print >>self.logfile,str(time.clock()),'ground_truth_generated.csv'       
...
#img = misc.imread(path)
    ...
    def MakeTestExpressions(self,tnumber):
        ...
        self.testexpr = [('in front','guy in the middle','guy or man in front middle')] + self.testexpr
    ...
    def hide(self):
        ...
        ref1 = [u[0] for u in self.testexpr]
        ref2 = [u[1] for u in self.testexpr]
        ref3 = [u[2] for u in self.testexpr]

        print len(tfiles),len(tregions),len(tsets),len(self.clicks),len(self.success),len(self.lastclick)
        logframe = pd.DataFrame({'file':tfiles,'region':tregions,'testset':tsets,'ref1':ref1,'ref2':ref2,'ref3':ref3,\
            'reftype':self.testref,\
